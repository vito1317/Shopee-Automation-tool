...
let ttsSequenceActive = false; 

function loadFeatureStates() {
    if (!chrome.runtime?.id) {
        console.warn("蝦皮自動化: Extension context invalidated at loadFeatureStates. Using default states.");
        const keys = Object.keys(featureStates);
        const defaultStates = {};
        keys.forEach(key => defaultStates[key] = featureStates[key]);
    ...
    chrome.storage.sync.get(keysToGet, (data) => {
        if (chrome.runtime.lastError) {
            console.warn("蝦皮自動化: Error loading feature states from storage. Using current/default states.", chrome.runtime.lastError.message);
        } else if (!chrome.runtime?.id) {
             console.warn("蝦皮自動化: Extension context invalidated in loadFeatureStates callback. Using current/default states.");
        }
        else {
...
function storageChangeListener(changes, namespace) {
    if (!chrome.runtime?.id) {
        console.warn("蝦皮自動化: Extension context invalidated in storage.onChanged listener. Attempting to remove listener.");
        try {
            if (chrome.storage && chrome.storage.onChanged) {
                chrome.storage.onChanged.removeListener(storageChangeListener);
            }
        } catch (e) {
            // console.warn("蝦皮自動化: Error removing storage.onChanged listener:", e);
        }
        return;
    }
        ...
        for (let key in changes) {
            if (featureStates.hasOwnProperty(key)) {
                ...
                if (typeof newValue === 'boolean') {
                    featureStates[key] = newValue;
                    statesChanged = true;
                }

                if ((key === 'masterEnabled' && !newValue) || (key === 'featureTTSEnabled' && !newValue)) {
                    ttsSequenceActive = false;
                    if (typeof speechSynthesis !== 'undefined' && speechSynthesis.speaking) {
                        speechSynthesis.cancel();
                    }
                }
                if (key === 'masterEnabled' && !newValue) {
...
function handleFeatureStateChange(isInitialLoad = false) {
    if (!featureStates.masterEnabled) {
        ...
        ttsSequenceActive = false; // Ensure TTS sequence is marked inactive
        if (typeof speechSynthesis !== 'undefined' && speechSynthesis.speaking) {
            speechSynthesis.cancel(); // Explicitly cancel speech
        }
        spokenPhrasesThisSession.clear(); // Clear spoken phrases if master is off
        return; 
    }
    ...
    if (featureStates.hasOwnProperty('featureTTSEnabled') && !featureStates.featureTTSEnabled) {
        ttsSequenceActive = false;
        if (typeof speechSynthesis !== 'undefined' && speechSynthesis.speaking) {
            speechSynthesis.cancel();
        }
...
function checkUrlAndResetStates(newUrl, isInitialLoad = false) {
    if (!chrome.runtime?.id && !isInitialLoad) { 
        console.warn("蝦皮自動化: Context invalidated in checkUrlAndResetStates.");
        return;
    }
...
if (chrome.runtime?.id) { 
    const masterUrlCheckInterval = setInterval(() => {
        if (!chrome.runtime?.id) {
            console.warn("蝦皮自動化: Context invalidated in masterUrlCheckInterval. Clearing interval.");
            clearInterval(masterUrlCheckInterval);
            return;
        }
...
if (chrome.runtime?.id) {
    const intervalId_BoxScan = setInterval(function() {
        if (!chrome.runtime?.id) {
            clearInterval(intervalId_BoxScan);
            return;
        }
        const newUrl = window.location.href;
...
function performAutoCallNumberLogic() {
    if (!chrome.runtime?.id) { stopAutoCallNumberFeature(); return; }
...
function startNextDayFeature() {
    ...
    nextDayIntervalId = setInterval(function() {
        if (!chrome.runtime?.id) { stopNextDayFeature(); return; }
...
function executeAfterTwoSscMessages() {
    if (!chrome.runtime?.id) { if(nextDayCheckInterval) clearInterval(nextDayCheckInterval); nextDayCheckInterval = null; return; }
    ...
    nextDayCheckInterval = setInterval(() => {
        if (!chrome.runtime?.id) { clearInterval(nextDayCheckInterval); nextDayCheckInterval = null; return; }
        checks++;
        ...
        if (ssc_messages.length >= 2) {
            ...
            setTimeout(() => {
                 if (!chrome.runtime?.id || !featureStates.masterEnabled || !featureStates.featureNextDayEnabled || !featureStates.featureNextDayAutoStartEnabled) return;
...
function startCheckoutDataProcessingAndTTS() {
    ...
    if (checkoutDataProcessingIntervalId === null) {
        ...
        setTimeout(() => { 
            if (!chrome.runtime?.id) return;
            processAndSpeakCheckoutData();
        }, 1500);
        checkoutDataProcessingIntervalId = setInterval(()=>{
            if (!chrome.runtime?.id) {
                stopCheckoutDataProcessingAndTTS();
                return;
            }
...
function processAndSpeakCheckoutData() {
    ...
    if (itemsToSpeakInOrder.length > 0 && featureStates.masterEnabled && featureStates.featureTTSEnabled) {
...
function speakTextArray(items) {
    ...
    let currentIndex = 0;
    function speakNext() {
        if (!chrome.runtime?.id) { ttsSequenceActive = false; return; }
        if (!ttsSequenceActive || !featureStates.masterEnabled || !featureStates.featureTTSEnabled) {
            ttsSequenceActive = false;
             if (typeof speechSynthesis !== 'undefined' && speechSynthesis.speaking) {
                speechSynthesis.cancel();
            }
        ...
        if (currentIndex < items.length) {
            ...
            utterance.onerror = (event) => {
                if (!ttsSequenceActive) return;
                console.error("蝦皮自動化: SpeechSynthesisUtterance 錯誤", event.error ? event.error : "未知錯誤");
                currentIndex++;
                setTimeout(speakNext, 300);
            };
...
(async function() {
    ...
    try {
        if (!chrome.runtime?.id) {
            console.warn("蝦皮自動化 (FileScan): Context invalidated at IIFE start. Aborting file scan setup.");
            return;
        }
        ...
        async function getFeatureStatesFromContent_IIFE() { 
            return new Promise((resolve) => {
                if (!chrome.runtime?.id) {
                    console.warn("蝦皮自動化 (FileScan IIFE): Context invalidated before storage.get. Using global featureStates.");
                ...
                chrome.storage.sync.get(keysToGet, (data) => {
                    if (chrome.runtime.lastError) {
                        console.warn("蝦皮自動化 (FileScan IIFE): Error in storage.get. Using global featureStates.", chrome.runtime.lastError.message);
                        resolve({ ...featureStates });
                    } else if (!chrome.runtime?.id) {
                        console.warn("蝦皮自動化 (FileScan IIFE): Context invalidated in storage.get callback. Using global featureStates.");
                        resolve({ ...featureStates });
                    } else {
                        resolve({ ...featureStates, ...data });
                    }
                });
            });
        }
        ...
        if (typeof pdfjsLib !== 'undefined') {
            if (!pdfjsLib.GlobalWorkerOptions.workerSrc && chrome.runtime?.id) {
                 try {
        ...
        const html5QrCodePath = chrome.runtime?.id ? chrome.runtime.getURL('libs/html5-qrcode.min.js') : '';
        ...
        function loadHtml5QrCode() {
            ...
            if (html5QrLoadAttempted || !html5QrCodePath) return;
        ...
        async function checkLibrariesAndInit() {
            if (!chrome.runtime?.id) {
                if(librariesCheckInterval) clearInterval(librariesCheckInterval);
                librariesCheckInterval = null;
                return;
            }
            ...
            librariesCheckInterval = setInterval(async () => {
                if (!chrome.runtime?.id) { clearInterval(librariesCheckInterval); librariesCheckInterval = null; return; }
                checkCounter++;
                const pdfOk=typeof pdfjsLib!=='undefined'&&pdfjsLib.GlobalWorkerOptions.workerSrc;
                const qrOk=typeof Html5Qrcode!=='undefined';
                const currentRuntimeFeatureStates = await getFeatureStatesFromContent_IIFE(); 
        ...
        async function processNextFileInQueue() {
            if (!chrome.runtime?.id) { fileQueue = []; isProcessing = false; return; }
            const currentRuntimeFeatureStates = await getFeatureStatesFromContent_IIFE();
            if (!currentRuntimeFeatureStates.masterEnabled || !currentRuntimeFeatureStates.featureFileScanEnabled) {
            ...
            try {
                ...
                const stateAfterProcessing = await getFeatureStatesFromContent_IIFE();
        ...
        async function simulateBarcodeInput(code, fileNameForReport) {
            if (!chrome.runtime?.id) throw new Error("Context Invalidated before simulateBarcodeInput");
            const currentRuntimeFeatureStates = await getFeatureStatesFromContent_IIFE();
            if (!currentRuntimeFeatureStates.masterEnabled || !currentRuntimeFeatureStates.featureFileScanEnabled) {
        ...
        async function processBarcodesAndQRCodes(pdf, processedCodesThisFile, statusPrefix, fileNameForReport) {
            if (!chrome.runtime?.id) { addBatchScanError(fileNameForReport, null, 'Scan', 'Context Invalidated', ''); return; }
            const currentRuntimeFeatureStates = await getFeatureStatesFromContent_IIFE();
            if (!currentRuntimeFeatureStates.masterEnabled || !currentRuntimeFeatureStates.featureFileScanEnabled) {
            ...
            const ps = Array.from({ length: pdf.numPages }, (_, i) => (async (pn) => {
                ...
                try {
                    let currentRenderState = await getFeatureStatesFromContent_IIFE();
                    ...
                    currentRenderState = await getFeatureStatesFromContent_IIFE();
                    if (!currentRenderState.masterEnabled || !currentRenderState.featureFileScanEnabled) {
            ...
            const finalState = await getFeatureStatesFromContent_IIFE();
            ...
            if (codesFoundByQRThisFunc.length > 0) {
                ...
                for (let i = 0; i < codesFoundByQRThisFunc.length; i++) {
                    ...
                    const simState = await getFeatureStatesFromContent_IIFE();
                ...
                const postSimState = await getFeatureStatesFromContent_IIFE();
        ...
        async function processPDF(file, statusPrefix, fileNameForReport) {
             return new Promise(async (resolve, reject) => {
                if (!chrome.runtime?.id) {
                    addBatchScanError(fileNameForReport, null, 'PDF', 'Context Invalidated', 'Pre-read');
                    updateStatusSpan(statusPrefix + `PDF處理中止: Context失效`, 'red', true);
                    resolve(); return;
                }
                ...
                reader.onload = async (e) => {
                    try {
                        if (!chrome.runtime?.id) {
                            addBatchScanError(fileNameForReport, null, 'PDF', 'Context Invalidated', 'Read complete');
                            updateStatusSpan(statusPrefix + `PDF處理中止: Context失效`, 'red', true);
                            resolve(); return;
                        }
                        let currentRuntimeFeatureStates = await getFeatureStatesFromContent_IIFE();
                        if(!currentRuntimeFeatureStates.masterEnabled||!currentRuntimeFeatureStates.featureFileScanEnabled){
                        ...
                        for (let i = 1; i <= pdfDoc.numPages; i++) {
                            currentRuntimeFeatureStates = await getFeatureStatesFromContent_IIFE();
                            if(!currentRuntimeFeatureStates.masterEnabled||!currentRuntimeFeatureStates.featureFileScanEnabled){
                        ...
                        if (!chrome.runtime?.id) {
                             addBatchScanError(fileNameForReport, null, 'PDF', 'Context Invalidated during processing', err.message);
                        } else {
                             addBatchScanError(fileNameForReport, null, 'PDF', 'Proc Err', err.message);
                        }
                ...
                reader.onerror = err => {
                    ...
                    reject(err); // Or resolve() if you want the queue to continue
                };
        ...
        async function processHTML(file, statusPrefix, fileNameForReport) {
            return new Promise((resolve, reject) => {
                if (!chrome.runtime?.id) {
                    addBatchScanError(fileNameForReport, null, 'HTML', 'Context Invalidated', 'Pre-read');
                    updateStatusSpan(statusPrefix + `HTML處理中止: Context失效`, 'red', true);
                    resolve(); return;
                }
                ...
                r.onload = async (e) => {
                    try{
                        if (!chrome.runtime?.id) {
                            addBatchScanError(fileNameForReport, null, 'HTML', 'Context Invalidated', 'Read complete');
                             updateStatusSpan(statusPrefix + `HTML處理中止: Context失效`, 'red', true);
                            resolve(); return;
                        }
                        let currentRuntimeFeatureStates = await getFeatureStatesFromContent_IIFE();
                        ...
                        resolve();
                         ...
                         if (!chrome.runtime?.id) {
                             addBatchScanError(fileNameForReport, null, 'HTML', 'Context Invalidated during processing', err.message);
                        } else {
        ...
        async function processImageFile(file, statusPrefix, fileNameForReport) {
            if (!chrome.runtime?.id) {
                addBatchScanError(fileNameForReport, null, 'Image', 'Context Invalidated', 'Pre-process');
                updateStatusSpan(statusPrefix + `圖像處理中止: Context失效`, 'red', true);
                return;
            }
            let currentRuntimeFeatureStates = await getFeatureStatesFromContent_IIFE();
            if (!currentRuntimeFeatureStates.masterEnabled || !currentRuntimeFeatureStates.featureFileScanEnabled) {
            ...
            // let imageFullyScanned = false; // Not strictly needed if we await the promise
            ...
            return new Promise((resolveFileProcessing) => {
                ...
                reader.onload = async (e) => {
                    try {
                        if (!chrome.runtime?.id) {
                            addBatchScanError(fileNameForReport, null, 'Image', 'Context Invalidated', 'Image read');
                            updateStatusSpan(statusPrefix + `圖像處理中止: Context失效`, 'red', true);
                            resolveFileProcessing(); return;
                        }
                        image.onload = async () => {
                            try {
                                if (!chrome.runtime?.id) {
                                    addBatchScanError(fileNameForReport, null, 'Image', 'Context Invalidated', 'Image loaded');
                                    resolveFileProcessing(); return;
                                }
                                ...
                                for (const q of quads) {
                                    if (!chrome.runtime?.id) { resolveFileProcessing(); return; }
                                    currentRuntimeFeatureStates = await getFeatureStatesFromContent_IIFE();
                                    if (!currentRuntimeFeatureStates.masterEnabled || !currentRuntimeFeatureStates.featureFileScanEnabled) {
                            ...
                            } catch (imgLoadProcessingError) {
                                 addBatchScanError(fileNameForReport, null, 'Image Process', 'Error after image load', imgLoadProcessingError.message);
                                 updateStatusSpan(statusPrefix + '圖像處理錯誤', 'red');
                                 resolveFileProcessing();
                            }
                        };
                        image.onerror = () => {
                        ...
                        if (!chrome.runtime?.id) { // Check before setting src
                             addBatchScanError(fileNameForReport, null, 'Image Load', 'Context Invalidated', 'Before setting src');
                             resolveFileProcessing(); return;
                        }
                        image.src = e.target.result;
                    } catch (fileReaderLoadError) {
                        addBatchScanError(fileNameForReport, null, 'Image Read', 'Outer error', fileReaderLoadError.message);
                        updateStatusSpan(statusPrefix + '圖像讀取外部錯誤', 'red');
                        resolveFileProcessing();
                    }
                };
                reader.onerror = () => {
                 ...
                 if (!chrome.runtime?.id) {
                     addBatchScanError(fileNameForReport, null, 'Image Read', 'Context Invalidated', 'Before readAsDataURL');
                     resolveFileProcessing(); return;
                 }
        ...
        if (e.message && e.message.toLowerCase().includes("extension context invalidated")) {
            console.warn("蝦皮自動化 (FileScan): IIFE caught an Extension context invalidated error. File scanning features may not work.", e);
        } else {
            console.error("蝦皮自動化 (FileScan): Error in IIFE setup.", e);
        }
    }
})();
