...
-   **🔀 自動叫號 (`自動叫號`)**
    -   在「排隊任務」頁面，自動點擊或聚焦「叫號」按鈕。
    -   可設定為「點擊」或僅「聚焦」(需手動按 Enter)。

-   **🛒 自動結帳 (`自動結帳`)**
    -   在「自助寄件出貨」頁面，當所有訂單都已備貨時，自動點擊或聚焦「完成出貨」按鈕。

-   **🔊 TTS 語音播報 (`TTS 語音播報`)**
    -   在「自助寄件出貨」頁面，自動朗讀關鍵資訊，減少看螢幕頻率。
    -   可獨立開關播報內容：
        -   **櫃位與末三碼**：例如「櫃位 B 12」、「末三碼 3 4 5」。
        -   **收款與找零金額**：例如「總金額 850 元」、「找 150 元」。

-   **📦 自動刷取物流箱 (`自動刷取物流箱單`)**
    -   在「包裹投遞」頁面，當聚焦於物流箱掃描框時，會自動填入 `BOX999999999` 並送出，節省手動掃描時間。

-   **✅ 自動完成與處理隔日件 (`自動完成隔日` & `隔日自動刷件`)**
    -   **自動裝箱**：在「包貨」頁面，聚焦於箱號輸入框時，自動填入箱號並點擊「完成包貨」。
    -   **一件一箱模式**：在「一件一箱」情境下，自動聚焦到條碼輸入框，實現快速連續作業。
